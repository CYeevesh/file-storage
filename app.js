  ...
  if (window.ethereum) {
    ...
    try {
      ...
      if (parseInt(networkId, 10) !== 11155111) {
        UIkit.notification({
          message:
            '<span uk-icon="icon: warning"></span> Please connect to the Sepolia Testnet.',
          status: "danger",
          pos: "top-center",
          timeout: 5000,
        });
        ...
        UIkit.notification({
          message:
            '<span uk-icon="icon: check"></span> Metamask wallet connected.',
          status: "success",
          pos: "top-center",
          timeout: 5000,
        });
      ...
      UIkit.notification({
        message:
          '<span uk-icon="icon: warning"></span> Please connect your MetaMask wallet.',
        status: "danger",
        pos: "top-center",
        timeout: 5000,
      });
    ...
    UIkit.notification({
      message: '<span uk-icon="icon: warning"></span> Please install MetaMask!',
      status: "danger",
      pos: "top-center",
      timeout: 5000,
    });
...
async function uploadFile() {
  ...
  const jwtToken =
    "eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9..D9CCodBH7YBGRCh_DOJHQQM8y8wrqKYyMpUi2BwTaLY";
  ...
  try {
    ...
    if (contract.methods.uploadFile) {
      await contract.methods
        .uploadFile(ipfsHash, encryptedKey)
        .send({ from: account });
...
async function fetchUploadedFiles() {
  ...
  try {
    ...
    files.forEach((file, index) => {
      ...
      cellHash.classList.add("uk-text-truncate");
...
async function fetchSharedFiles() {
  const sharedFilesTableBody = document
    .getElementById("usersSharedFilesTable")
    .getElementsByTagName("tbody")[0];
  ...
  try {
    ...
    files.forEach((file, index) => {
      ...
      cellOwner.classList.add("uk-text-truncate");
      cellKey.textContent = file.encryptedKey;
      ...
      downloadButton.innerHTML = '<span uk-icon="icon: download"></span>';
...
async function sharedFilesList() {
  const sharingListTableBody = document
    .getElementById("sharingListTable")
    .getElementsByTagName("tbody")[0];
...
async function grantPermission() {
  ...
  const grantPermissionStatus = document.getElementById(
    "grantPermissionStatus"
  );

  if (!fileIndex || !userAddress) {
    grantPermissionStatus.textContent =
      "Please provide both file index and user address.";
    return;
  }
  ...
  try {
    ...
    if (contract.methods.grantPermission) {
      await contract.methods
        .grantPermission(fileIndex, userAddress)
        .send({ from: account });
      ...
      grantPermissionStatus.textContent =
        "grantPermission method not found in contract";
    }
  } catch (error) {
    console.error(error);
    grantPermissionStatus.textContent =
...
async function revokePermission() {
  ...
  try {
    ...
    if (contract.methods.revokePermission) {
      await contract.methods
        .revokePermission(fileIndex, userAddress)
        .send({ from: account });
      ...
      permissionStatus.textContent =
        "revokePermission method not found in contract";
    }
  } catch (error) {
    console.error(error);
    permissionStatus.textContent =
